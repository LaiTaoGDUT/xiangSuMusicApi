...
async function get(path, query, querystring) {
    ...
    const res = await new Promise((resolve, reject) => {
        superagent.get(_url).set("Cookie",global.user_cookie).end(function (err, res) {
            if (!err) {
...
async function post(path, query, querystring, body) {
    ...
    const res = await new Promise((resolve, reject) => {
        superagent.post(_url).set("Cookie",global.user_cookie).send(body).end(function (err, res) {
            if (!err) {
                access(`${_url} -- ${new Date()}}`);
                resolve(res.body);
            } else {
